% % % % % % % % % AERO3260 Aerodynamics 1 Assignment 2 % % % % % % % % % %
% Author: 470390467


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                           READ ME
%  This script is set up to automatically run a range of different programs
%  Change the 'mode' variable to change the program.
%  program description shown below (line 44)
%  To run the current inputs below, set mode = 0
%  The main VLM() function begins on line 311
%  Other subroutines can be found below VLM() function
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Geom.twist = 0;         
Geom.dihed = 0;
Geom.camber = 0; % 0 == Flat Plat, 1 == NACA 65(2)-415, 2 == NACA 23009

% Other Variables
alpha = 5;              % AoA (deg)
npx = 11;               % Number of x points
npy = 21;               % Number of y points
toPlot = 1;             % Plotting flag
...
%                     Select mode to run 
%              MODE: 
%                   0 = Runs input from above
%                   1 = Wing Sweep geometric investigation 
%                   2 = Taper Ratio geometric investigation 
%                   3 = Dihedral Angle geometric investigation 
%                   4 = Wing Tip Twist geometric investigation 
%                   5 = Resolution Study (Takes a ~ 3 min)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                 mode = 0
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
switch mode
    ...
    % SR22 Wing Platform
    case 0
        disp('Running Geometry Inputs from line 21-37')
        % Run the main function
        [CL, Cdi, Cl_section, Cl_CL] = VLM(Geom, alpha, U, rho, npx, npy, L, toPlot);
    ...
    case 1
        ...
        Geom.dihed = 0;    
        Geom.camber = 0;
        y_b = linspace(0,1,npy-1);
        figure()
        hold on
        for i = 1:length(sweep)
            Geom.sweep = sweep(i);
            [~,~,~,Cl_CL] = VLM(Geom, alpha, U, rho, npx, npy, L, 0);
            ClCL_vec(:,i) = Cl_CL;
            plot(y_b, Cl_CL)
        end
        xlabel('y/b')
        ylabel('Cl/CL')
        legend('0 deg \Lambda','45 deg \Lambda','135 deg \Lambda')
        grid on
        dim = [0.4 .5 .3 .3];
        str = 'AR = 4.0';
        ...
        for ar = 1:length(b)
            ...
            for i = 1:length(sweep)
                Geom.sweep = sweep(i);
                Cla_vec(i) = LiftCurveSlope(Geom,U,npx,npy,L,rho);
                [~, Cdi, ~, ~] = VLM(Geom, alpha, U, rho, npx, npy, L, 0);
                Cd_vec(i) = Cdi;
            end
        figure(2)
        hold on
        plot(sweep,Cla_vec,'-o')
        figure(3)
        hold on
        plot(sweep, Cd_vec,'-o')
        end
        figure(3)
        legend('3 AR', '6 AR', '9 AR','location','best')
        grid on
        xticks(sweep)
        xlabel('\Lambda (deg)')
        ylabel('C_{D_i}')
    ...
    case 2
        ...
        xlabel('y/b')
        ylabel('Cl/CL')
        legend('0.1 \lambda','0.4 \lambda','0.6  \lambda','1.0 \lambda','location','NorthWest')
        grid on
        dim = [0.4 .5 .3 .3];
        str = 'AR = 7.28';
        annotation('textbox',dim,'String',str,'FitBoxToText','on');
        ...
        figure(2)
        plot(lambda,Cla_vec,'-o')
        hold on
        figure(3)
        plot(lambda,Cd_vec,'-o')
        hold on
        end
        figure(2)
        legend('3 AR', '6 AR', '9 AR','location','best')
    ...
    case 3
        ...
        xlabel('y/b')
        ylabel('Cl/CL')
        legend('-20 deg \gamma','0 deg \gamma','20 deg \gamma','location','Best')
        grid on
        dim = [0.3 .5 .3 .3];
        str = 'AR = 4';
        annotation('textbox',dim,'String',str,'FitBoxToText','on');
        b = [3, 6, 9];
        dihed = [-20,-15,-10,-5,0,5,10,15,20]; 
        figure()
        hold on
        for ar = 1:length(b)
            Geom.b = b(ar);
    ...
    case 4
        ...
        xlabel('y/b')
        ylabel('Cl/CL')
        legend('-15 deg twist', '0 deg twist', '15 deg twist','30 deg twist','location','Best')
        grid on
        dim = [0.4 .5 .3 .3];
        str = 'AR = 4';
        annotation('textbox',dim,'String',str,'FitBoxToText','on');
    ...
    % Resolution Study 
    case 5
        npx = [2,5, 10, 15, 20,30,40,50];
        npy = [3,10, 20, 30, 40,50,60,70];
        L = (npx-1).*(npy-1);
        tic
        tStart = tic;
        T(1) = toc;
        for i = 1:length(npx)
            [CL,Cdi,~,~] = VLM(Geom, alpha, U, rho, npx(i), npy(i), L(i), 0);
            CL_vec(i) = CL;
            Cdi_vec(i) = Cdi;
            T(i+1) = toc;
        end
        T_vec = diff(T);
        ResolutionStudy = [npx',npy',L',CL_vec',[NaN;diff(CL_vec)'],Cdi_vec',[NaN;diff(Cdi_vec)'],T_vec'];
end
    


function [CL, Cdi, Cl_section, Cl_CL] = VLM(Geom, alpha, U, rho, npx, npy, L, toPlot)
%% MAIN
% Calls all other functions
    ...
    RHS = GenFreeStreamMat(Geom, U, alpha, Normals_Cell, L);
    ...
    if toPlot
        ...
        ylabel('Cl/CL')
        xlabel('y/b')
        title('Sectional Lift Coefficient vs y/b')
        grid on
    end
end 
...
function RHS = GenFreeStreamMat(Geom, U, alpha, Normals_Cell, L)
...
% Correct? YES 
    ...
    v_inf = 0; 
    w_inf= U*sin(alpha);
    if Geom.camber == 1 % If including angle of attack (from camber)
    	 w_inf= U/1.5*sin(alpha);
    end
...
% Author: 470 390 467
    ...
    p1 = polyfit(data_1(:,1),data_1(:,2),3);
    p2 = polyfit(data_2(:,1),data_2(:,2),3);
    P = (p1 + p2)/2;
    x = linspace(0,1,npx);
    z_c = polyval(P,x);
...
function Cla = LiftCurveSlope(Geom,U,npx,npy,L,rho)
%% Calculates the lift curve slope 
    alpha = 1; % Because Lift curve slope is proportional 
    [CL, ~, ~, ~] = VLM(Geom, alpha, U, rho, npx, npy, L, 0);
    Cla = CL*180/pi;
end
...
% Woo hoo you made it to the end 
